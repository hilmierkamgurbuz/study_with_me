...
- Any code touching a UnityEngine API (AudioSource, AudioClip, UI events) from the WebSocket receive path must be marshaled onto the main thread first (a queue drained in `Update()`); the receive loop itself runs on a background thread.
